...
class TestModelGeneration(unittest.TestCase):
    ...
    def test_model14_solver_error(self):
        ...
        voltage_time_series = np.array([voltage_func(t) for t in times])
        current = np.full(voltage_time_series.shape, 1)
        boundaries = markovmodels.fitting.fitting_boundaries(full_parameters,
                                                             model, current,
                                                             voltage_time_series,
                                                             rng)
